...
const CategoryForm: React.FunctionComponent<ICategoryFormProps> = ({ formType, requestPath, id }) => {
    ...
    useEffect(() => {
        if (id) {
            console.log(id)
            axios.get('/categories/' + id).then(res => setCategory(res.data.name)).catch(e => setError([...err, e]))
        }
    }, [])
