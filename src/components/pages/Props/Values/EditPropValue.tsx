...
import { Container, CssBaseline, Box, Typography, Button, FormControl, Input, InputLabel } from '@mui/material'
...
const EditPropValue: React.FC = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`/props/values/${valueId}`).then(res => { setValue(res.data.value); }).catch(error => setError(prev => {
            if (!prev) {
                return [error]
            }
            return [...prev, error]
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const opts = { url: '/props/values/update/' + valueId, method: "put", data: { value } }
